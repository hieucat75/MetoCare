...
/**
 * Single-metric clinical insight (meaning / risks / lifestyle / follow-up).
 * Thin wrapper over the EXISTING backend endpoint `GET /patients/{id}/insights/{metric_type}`
 * (see the insights list endpoint above). Returns `null` on failure or when the
 * metric has no insight (e.g. derived BMI, or the feature flag is off), so the
 * detail screen degrades gracefully instead of crashing.
 */
export async function getInsight(
  ...
  try {
    return await api.get<MetricInsight>(
      `/patients/${patientId}/insights/${encodeURIComponent(metricType)}`
    )
  } catch {
    return null
  }
