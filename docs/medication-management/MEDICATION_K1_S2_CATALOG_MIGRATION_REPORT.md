...
Full list (generic_name → active_ingredient → drug_class), **queried directly from the migrated database, not transcribed from memory** — a first draft of this report had exactly the kind of guessed-salt-form errors this migration is designed to avoid (e.g. wrote "pioglitazone→pioglitazone" when the true migrated value is "pioglitazone hydrochloride"); Codex review caught it:

allopurinol→allopurinol→xanthine_oxidase_inhibitor, amlodipine→amlodipine besilate→calcium_channel_blocker, apixaban→apixaban→noac, aspirin→acetylsalicylic acid→antiplatelet, atorvastatin→atorvastatin calcium trihydrate→statin, bisoprolol→bisoprolol fumarate→beta_blocker, candesartan→candesartan cilexetil→arb, clopidogrel→clopidogrel bisulphate→antiplatelet, colchicine→colchicine→anti_inflammatory_gout, dapagliflozin→dapagliflozin propanediol→sglt2_inhibitor, empagliflozin→empagliflozin→sglt2_inhibitor, esomeprazole→esomeprazole magnesium→proton_pump_inhibitor, essential phospholipids→polyenylphosphatidylcholine→hepatoprotective_supplement, ezetimibe→ezetimibe→cholesterol_absorption_inhibitor, febuxostat→febuxostat→xanthine_oxidase_inhibitor, fenofibrate→fenofibrate→fibrate, gliclazide→gliclazide→sulfonylurea, glimepiride→glimepiride→sulfonylurea, hydrochlorothiazide→hydrochlorothiazide→thiazide_diuretic, indapamide→indapamide→thiazide_like_diuretic, insulin aspart→insulin aspart→rapid_acting_insulin, insulin glargine→insulin glargine→long_acting_insulin, levothyroxine→levothyroxine sodium→thyroid_hormone, losartan→losartan potassium→arb, metformin→metformin hydrochloride→biguanide, metoprolol→metoprolol succinate→beta_blocker, nebivolol→nebivolol hydrochloride→beta_blocker, omeprazole→omeprazole→proton_pump_inhibitor, pantoprazole→pantoprazole sodium sesquihydrate→proton_pump_inhibitor, perindopril→perindopril arginine→ace_inhibitor, pioglitazone→pioglitazone hydrochloride→thiazolidinedione, rivaroxaban→rivaroxaban→noac, rosuvastatin→rosuvastatin calcium→statin, silymarin→silymarin→hepatoprotective_supplement, simvastatin→simvastatin→statin, sitagliptin→sitagliptin phosphate monohydrate→dpp4_inhibitor, spironolactone→spironolactone→aldosterone_antagonist, telmisartan→telmisartan→arb, valsartan→valsartan→arb, vildagliptin→vildagliptin→dpp4_inhibitor, warfarin→warfarin sodium→vitamin_k_antagonist.
...
**None.** No entry had a combination-product ingredient list, a name collision with another entry, or any other identity ambiguity that would require a human decision before migrating. If this changes in a future catalog update (e.g. a combination product is added), this migration's one-ingredient-per-row assumption (`row.active_ingredients[0]`) would need revisiting — the migration now **raises `RuntimeError` and refuses to run** if any source row has `len(active_ingredients) != 1` (Codex review caught that the original version would have silently truncated a combination product to its first ingredient instead of failing loudly).
