...
_MOCK_SUMMARIES: dict[ExplanationType, str] = {
    ExplanationType.metabolic_score: (
        "Your metabolic wellness score reflects how your key health metrics are "
        "working together. A higher score means your body is managing important "
        "factors like blood sugar and blood pressure more efficiently. Keep up "
        "your current healthy habits and check back regularly to see your progress."
    ),
...
def explain(
    ...
    if payload.explanation_type == ExplanationType.health_metric and ctx.metric_type:
        ...
        trend_str = f" The trend looks {ctx.trend}." if ctx.trend else ""
        summary = (
            f"Your metabolic wellness score is {ctx.score}."
            + trend_str
