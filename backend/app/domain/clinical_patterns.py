...
def detect_patterns(summary: dict[str, object]) -> list[PatternDetection]:
    ...
    if (
        derived.get("tyg_index") is not None and float(derived["tyg_index"]) > 9.0
        and has("fasting_glucose") and has("triglyceride") and has("hdl")
    ):
        patterns.append(PatternDetection(
            ...
            _confidence(3, True), True, "established",
        ))
    ...
    if isinstance(ms, dict) and ms.get("status") == "meets_criteria":
        patterns.append(PatternDetection(
            ...
            "warning",
            ["metabolic_syndrome"],
            "high" if int(ms.get("criteria_count", 0)) >= 3 else "low", True, "established",
        ))

    if (
        (derived.get("ldl_friedewald") is not None and float(derived["ldl_friedewald"]) > 3.4)
        or (derived.get("non_hdl_cholesterol") is not None and float(derived["non_hdl_cholesterol"]) > 4.1 and has("triglyceride"))  # noqa: E501
        ...
        patterns.append(PatternDetection(
            ...
            "watch",
            [k for k in ["ldl_friedewald", "non_hdl_cholesterol", "triglyceride", "hdl"] if has(k)],
            "high" if sum(int(has(k)) for k in ["ldl_friedewald", "non_hdl_cholesterol", "triglyceride", "hdl"]) >= 2 else "low",  # noqa: E501
            False, "established",
        ))

    if has("alt") and has("triglyceride") and (has("fasting_glucose") or summary.get("bmi_proxy") is not None):  # noqa: E501
        patterns.append(PatternDetection(
            "fatty_liver_pattern",
            ...
            _confidence(2, False), True, "moderate",
        ))
