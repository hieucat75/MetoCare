...
class TestPublishedQueryExcludesDrafts:
    ...
    def test_zero_approved_rows_exist_anywhere(self, db) -> None:
        """K1-S3 scope lock: nothing in this codebase can create an approved
        row — verify no approved DrugUsage row exists at all, not just none
        matching one specific ingredient."""
        count = db.query(DrugUsage).filter_by(status="approved").count()
        assert count == 0
