...
def test_patient_cannot_read_another_patients_history(client: TestClient, patient_a, patient_b):
...
def test_red_flags_serialized_correctly(client: TestClient, db, patient_a):
    ...
    history = client.get(_history_url(patient_a["patient_id"]), headers=patient_a["headers"])
...
def test_triage_history_ordered_newest_first(client: TestClient, db, patient_a, admin_headers):
