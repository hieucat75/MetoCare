...
RESP=$(curl -sf "$BASE/api/v1/info" 2>/dev/null)
echo "$RESP" | python3 -c "
import sys, json
d = json.load(sys.stdin)
assert d.get('app'), 'missing app field'
print('  env=%s ai_mode=%s' % (d.get('env','?'), d.get('ai_mode','?')))
" 2>/dev/null && ok "GET /api/v1/info → fields present" || fail "GET /api/v1/info failed: $RESP"

hdr "ITEM 3 — Patient login"
RAW=$(curl -sf -w "\n%{http_code}" -X POST "$BASE/api/v1/auth/login" \
    -H "Content-Type: application/json" \
    -d '{"email":"demo.patient@example.com","password":"DemoPatient123!"}' 2>/dev/null)
...
[[ "$HTTP" == "200" && -n "$PAT_TOKEN" ]] \
    && ok "Patient login → 200, token obtained" \
    || fail "Patient login HTTP=$HTTP body=$BODY"
...
if [[ -n "$PAT_TOKEN" ]]; then
    ...
    HTTP=$(echo "$RAW" | tail -1)
    BODY=$(echo "$RAW" | head -1)
    PAT_ID=$(echo "$BODY" | python3 -c "import sys,json; d=json.load(sys.stdin); print(d.get('patient_profile_id','') or d.get('id',''))" 2>/dev/null)
    [[ "$HTTP" == "200" && -n "$PAT_ID" ]] \
        && ok "GET /me → 200, patient_profile_id=$PAT_ID" \
        || fail "GET /me HTTP=$HTTP"
else
...
if [[ -n "$PAT_ID" ]]; then
    ...
    HTTP=$(echo "$RAW" | tail -1)
    [[ "$HTTP" == "200" ]] && ok "GET /profile → 200" || fail "Profile HTTP=$HTTP"
else
    skip "No patient_id"
fi
...
if [[ -n "$PAT_ID" ]]; then
    DATA="{\"patient_id\":\"$PAT_ID\",\"explanation_type\":\"general_summary\",\"context\":{}}"
    RAW=$(curl -sf -w "\n%{http_code}" -X POST "$BASE/api/v1/ai/explain" \
        -H "Content-Type: application/json" \
    ...
    BODY=$(echo "$RAW" | head -1)
    [[ "$HTTP" == "200" ]] \
        && ok "POST /ai/explain → 200 (mock)" \
        || fail "AI explain HTTP=$HTTP body=${BODY:0:120}"
else
    skip "No patient_id"
fi
...
if [[ -n "$PAT_TOKEN" ]]; then
    RAW2=$(curl -sf -w "\n%{http_code}" -X POST "$BASE/api/v1/auth/login" \
        -H "Content-Type: application/json" \
        -d '{"email":"demo.patient@example.com","password":"DemoPatient123!"}' 2>/dev/null)
    REFRESH_TOKEN=$(echo "$RAW2" | head -1 | python3 -c "import sys,json; d=json.load(sys.stdin); print(d.get('refresh_token',''))" 2>/dev/null)
    if [[ -n "$REFRESH_TOKEN" ]]; then
        RAW3=$(curl -sf -w "\n%{http_code}" -X POST "$BASE/api/v1/auth/refresh" \
            -H "Content-Type: application/json" \
            -d "{\"refresh_token\":\"$REFRESH_TOKEN\"}" 2>/dev/null)
        HTTP3=$(echo "$RAW3" | tail -1)
        [[ "$HTTP3" == "200" ]] \
            && ok "POST /auth/refresh → 200" \
            || fail "Token refresh HTTP=$HTTP3"
    else
...
RAW=$(curl -sf -w "\n%{http_code}" -X POST "$BASE/api/v1/auth/login" \
    -H "Content-Type: application/json" \
    -d '{"email":"demo.doctor@example.com","password":"DemoDoctor123!"}' 2>/dev/null)
HTTP=$(echo "$RAW" | tail -1)
BODY=$(echo "$RAW" | head -1)
DOC_TOKEN=$(echo "$BODY" | python3 -c "import sys,json; d=json.load(sys.stdin); print(d.get('access_token',''))" 2>/dev/null)
[[ "$HTTP" == "200" && -n "$DOC_TOKEN" ]] \
    && ok "Doctor login → 200, token obtained" \
    || fail "Doctor login HTTP=$HTTP"
...
RAW=$(curl -sf -w "\n%{http_code}" -X POST "$BASE/api/v1/auth/login" \
    -H "Content-Type: application/json" \
    -d '{"email":"demo.admin@example.com","password":"DemoAdmin123!"}' 2>/dev/null)
HTTP=$(echo "$RAW" | tail -1)
BODY=$(echo "$RAW" | head -1)
ADM_TOKEN=$(echo "$BODY" | python3 -c "import sys,json; d=json.load(sys.stdin); print(d.get('access_token',''))" 2>/dev/null)
[[ "$HTTP" == "200" && -n "$ADM_TOKEN" ]] \
    && ok "Admin login → 200, token obtained" \
    || fail "Admin login HTTP=$HTTP"
...
# Next.js middleware does client-side redirect (307, no Location header);
# 307 from Next.js standalone = server is up and responding correctly
HTTP=$(curl -s -o /dev/null -w "%{http_code}" "$FRONT/" 2>/dev/null)
[[ "$HTTP" == "200" || "$HTTP" == "307" || "$HTTP" == "308" ]] \
    && ok "GET $FRONT/ → HTTP $HTTP (Next.js up)" \
    || fail "Frontend HTTP=$HTTP"
...
echo "════════════════════════════════════════"
[[ $FAIL -eq 0 ]] \
    && echo "  VERDICT: ALL CLEAR — stack ready for internal use" \
